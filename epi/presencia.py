...
@cache_region('default_term', 'getMarcatges')
def getMarcatges(epiapi):
    """
    ...
    """
    log(epiapi, "getMarcatges")
    return getMarcatgesBase(MARCATGES_URL)

def getMarcatgesBase(epiapi, URL, **kwargs):
    """
    """
    now = DateTime().latestTime()
    ...
    if is_historic_query:
        ...
        dies = deepcopy(getPermisos(epiapi, epiapi.username))
        if now.month()<3:
    ...
    # br=Browser()  Parxejat per que originariament venia com a atribut de la instancia
    marcatges = epiapi.br.open(URL)
    marcatges_html = marcatges.read()
    if checkBrowserExpired(marcatges_html):
        return 'EXPIRED'
    ...
    for dia in table.findChildren(recursive=False):
       ...
       if collapse:
          ...
          lusername = epiapi.username.replace('.',' ')
       ...
       if not dia.findAll('table') and dia.td and meves:
           data,marcatge_dict = parseDia(dia)
...
def getPermisos(epiapi, username):
    """
    """
    log(epiapi, "getPermisos")
    return getPermisosBase(epiapi, PERMISOS_URL)

def getPermisosBase(epiapi, url):
    """
    """
    now = DateTime().latestTime()
    dies_permisos = []
    ...
    # br=Browser() # Parxejat per que originariament venia com a atribut de la instancia
    permisos = epiapi.br.open(url)
    permisos_html = permisos.read()
    if checkBrowserExpired(permisos_html):
        return 'EXPIRED'
    ...
    try:
      ...
      #La sessio del browser ha caducat, per tant refarem el login
        login(epiapi, message="Re-Logging-in into presència via regular login")
        permisos = epiapi.br.open(PERMISOS_URL)
        permisos_html = permisos.read()
        permisos.close()
        soup = BeautifulSoup(permisos_html,fromEncoding='iso-8859-1')
        try:
    ...
    for dia in table.findChildren(recursive=False):
       ...
       if not dia.findAll('table') and dia.td and meves:
           ...
           if 'colspan' in attrmap.keys():
               ...
               permis_data = parsePermis(dia)
               dies_permisos = dies_permisos+generarDiesPermisos(permis_data,motiu)
...
def log(epiapi, message):
    """
    """
    logger = logging.getLogger('RUNNING')
    logger.info('%s - %s' % (epiapi.username,message))

def login(epiapi, message = "Logging-in into presència via regular login"):
    """
    Es logueja a presència amb el login tradicional web
    """
    log("Presència Login %s" % message)
    
    # br=Browser() # Parxejat per que originariament venia com a atribut de la instancia  
    epiapi.br.open(LOGIN_URL)
    epiapi.br.select_form(nr=0)
    epiapi.br['Username']=epiapi.username
    epiapi.br['Password']=epiapi.password
    response = epiapi.br.submit()
    response_html = response.read()
    response.close()
    saveSessionData(epiapi)

def checkBrowserExpired(html):
    """
...
def generarDiesPermisos(permis,motiu):
    """
    ...
    if permis['approved']:
        ...
        while grow_days:
            ...
            dhfd = getDiscountHoursForDay(DateTimeToTT(current),7)
...
def getDiscountHoursForDay(dia,hores_dia):
    day = '%s-%s-%s' % (dia)
    years={'2010':[],'2011':[]}
    years['2010']= {
                   '01-01-2010':'F',
...
def parseDia(dia):
    ...
    now = DateTime().latestTime() # Parxejat per a que agafi un now local
    ...
    if marcatges_web:
        ...
        if len(novamarca)==1:
            ...
            ultimamarca = now!=novamarca_latest and novamarca_latest or None
...
def getBrowserSession(epiapi):
    """
    Retorna la sessio actual del browser per a poderla guardar desde la utility
    """

    cookies = []
    for key in epiapi.br._ua_handlers['_cookies'].cookiejar._cookies.keys():
       domain = epiapi.br._ua_handlers['_cookies'].cookiejar._cookies[key]
...
def closeBrowser(epiapi):
    """
    """
    epiapi.br.close()

def saveSessionData(epiapi):
    """
    """
    epiapi.epiUtility.saveBrowserSession(epiUtility.request, epiUtility.username, getBrowserSession(epiapi), 'presencia')
    return
